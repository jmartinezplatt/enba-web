# Guia de Voz de Marca - Espacio Nautico Buenos Aires

## 1. Identidad de Marca

**Nombre completo**: Espacio Nautico Buenos Aires
**Sigla**: ENBA
**Tagline principal**: "Navega el Rio de la Plata"
**Taglines alternativos**:
- "Navega donde quieras, como quieras"
- "Tu puerta al Rio de la Plata"
- "Viento, agua y libertad"

---

## 2. Personalidad de Marca

La voz narradora de ENBA es **profesional, apasionada y con caracter femenino**: segura, calida, directa. Habla con la autoridad de quien conoce el agua de adentro. Su entusiasmo por la nautica es genuino y se nota, pero siempre dentro de un marco de elegancia y confianza. El negocio es para todo publico; la voz es el estilo de comunicacion de ENBA, no una segmentacion de audiencia.

ENBA es un espacio abierto a cualquier persona que quiera conectar con el agua.

### Atributos de personalidad

| Somos | No somos |
|-------|----------|
| Apasionados por navegar | Vendedores agresivos |
| Profesionales y seguros | Rigidos o corporativos |
| Cercanos y accesibles | Exclusivistas o elitistas |
| Aventureros con criterio | Temerarios o irresponsables |
| Conocedores del rio | Pedantes o tecnicos en exceso |
| Argentinos, rioplatenses | Genericos o sin identidad local |

### 4 Lineas de Servicio

| Linea | Emocion clave |
|-------|---------------|
| Compra/Venta/Alquiler de embarcaciones | Libertad, experiencia inmediata, accesibilidad, inversion inteligente |
| Escuela de navegacion y vela | Aprendizaje, logro, comunidad, transformacion |
| Turismo y excursiones nauticas | Emocion, descubrimiento, aventura nautica |
| Servicio tecnico a embarcaciones | Confianza, cuidado, mantenimiento profesional, respaldo tecnico |

---

## 3. Tono de Comunicacion

### General
- **Cercano** pero profesional
- **Entusiasta** sin ser exagerado
- **Informativo** sin ser aburrido
- **Inspirador** sin ser cursi
- **Argentino** sin ser lunfardo extremo
- **Maximo 2-3 emojis** por post, siempre con intencion
- **CTA claro y elegante** al final de cada post
- **Nunca** prometes disponibilidad sin confirmacion del equipo

### Por plataforma

| Plataforma | Tono | Ejemplo |
|------------|------|---------|
| **Instagram** | Aspiracional, estetico. Captions 80-150 palabras. | "Esto es salir de Puerto Norte un viernes. Te animas?" |
| **Facebook** | Informativo, calido, orientado a comunidad. Hasta 200 palabras. Unico formato donde se listan formas de pago. | "Tu embarcacion merece el mejor cuidado. Servicio tecnico profesional en Puerto Norte." |
| **TikTok** | Dinamico, gancho en las primeras 2 lineas. Max 80 palabras. Subtitulos en pantalla siempre. | "POV: estas a 3 horas de Buenos Aires pero parece otro mundo" |
| **WhatsApp** | Directo, servicial, rapido | "Hola! Gracias por escribirnos. Te paso info..." |

### Voseo
Usamos **voseo rioplatense** siempre:
- "Navega" -> "Navega" (imperativo con vos)
- "Reserva tu lugar" -> "Reserva tu lugar"
- "Animate a navegar" -> "Animate a navegar"
- "Conoce nuestros veleros" -> "Conoce nuestros veleros"

---

## 4. Vocabulario de Marca

### Palabras que usamos

| Categoria | Palabras clave |
|-----------|---------------|
| Navegacion | velero, navegar, travesia, salida, fondeo, virada, proa, popa, babor, estribor |
| Destinos nacionales | Delta, Isla Martin Garcia, Mar del Plata |
| Destinos internacionales | Colonia del Sacramento, Carmelo, Riachuelo, Juan Lacaze |
| Experiencia | aventura, escapada, libertad, horizonte, atardecer, viento |
| Educacion | timonel, patron, curso, certificado, Prefectura, maniobra |
| Comunidad | tripulacion, a bordo, zarpar, camaraderia |
| Confianza | seguridad, experiencia, flota nueva, instructores certificados |

### Palabras que NO usamos

| Evitar | Por que | Usar en su lugar |
|--------|---------|-----------------|
| "Lujo" | Somos accesibles, no elitistas | "Experiencia unica" |
| "Barato" | Desvaloriza el servicio | "Accesible", "al alcance" |
| "Crucero" | No somos cruceros | "Travesia", "navegacion" |
| "Yate" | Excepto en "patron de yate" | "Embarcacion", "velero" |
| "Paseo" | Minimiza la experiencia | "Travesia", "navegacion" |
| "Tour" | Muy turistico generico | "Travesia", "escapada nautica" |
| "Cliente" (en posts) | Muy comercial | "Tripulante", "navegante" |
| "Puerto Madero" | PROHIBIDO - ubicacion incorrecta | "Puerto Norte, frente al Aeroparque" |
| "Equipos nauticos" | Ya no vendemos equipos | "Servicio tecnico a embarcaciones" |
| Precios exactos | Nunca sin aprobacion del equipo | "Consulta por DM o WhatsApp" |

### REGLAS CRITICAS (del Prompt Maestro v1.3)

Lo que el agente/community manager NUNCA hace:
- Publicar precios exactos sin aprobacion del equipo de ENBA
- Confirmar fechas o disponibilidad sin verificacion previa
- Mencionar condiciones financieras de las cuotas
- Usar un tono sensacionalista o hacer promesas exageradas
- Ignorar comentarios negativos (se derivan al equipo)
- Segmentar el contenido por genero
- Decir "Puerto Madero" como punto de salida (es Puerto Norte)

Si preguntan por precio: "Podemos brindarte la informacion por mensaje directo o WhatsApp" y derivar.

---

## 5. Estructura de Captions

### Formula basica: HOOK + VALOR + CTA

```
[HOOK: primera linea que atrapa]

[CUERPO: 2-3 lineas con valor, info o historia]

[CTA: llamada a la accion]

[HASHTAGS: bloque al final]
```

### Ejemplos por tipo de post

**Travesia (aspiracional)**:
```
Asi se ve Buenos Aires cuando se achica en el horizonte.

Travesia a Colonia, 60 millas nauticas de viento y agua.
Salimos el proximo sabado. Quedan 3 lugares.

Reserva por DM o en espacionautico.com.ar/travesias

#TravesiaEnVelero #RioDeLaPlata #Colonia #VelerosBuenosAires
```

**Escuela (educativo)**:
```
Sabias que con el carnet de timonel podes navegar veleros de hasta 8 metros?

En ENBA te formamos en 3 meses con clases teoricas y practicas en el Rio
de la Plata. Instructores certificados por Prefectura Naval.

Inscripciones abiertas. Link en bio.

#EscuelaNautica #Timonel #CursoNautico #BuenosAires
```

**Velero en venta (comercial)**:
```
MASTI - Listing nuevo

Velero X pies, ano Y. Ideal para crucero costero y travesias.
Motor nuevo, velas en excelente estado, electronica actualizada.

Todos los detalles en espacionautico.com.ar/veleros-en-venta/masti

#VeleroEnVenta #Broker #VelerosBuenosAires #Sailing
```

---

## 6. Hashtags Estrategicos

### Hashtags fijos de marca (usar siempre)
```
#EspacioNautico #ENBA #NavegaElRioDeLaPlata
```

### Hashtags por categoria

**Travesias**:
```
#TravesiaEnVelero #VelerosBuenosAires #RioDeLaPlata
#NavegacionAVela #SailingArgentina #EscapadaNautica
#Colonia #Delta #Montevideo #PuntaDelEste
```

**Escuela**:
```
#EscuelaNautica #CursoDeTimonel #PatronDeYate
#AprenderANavegar #PrefecturaNaval #NauticaArgentina
```

**Broker**:
```
#VeleroEnVenta #ComprarVelero #BrokerNautico
#VelerosUsados #SailboatForSale #YachtBroker
```

**Generales / alcance**:
```
#BuenosAires #Palermo #PuertoNorte #Argentina
#Sailing #SailingLife #Sailboat #Wind #Sea
#Aventura #Libertad #Atardecer #NaturalezaArgentina
```

### Reglas de hashtags
- Instagram: 8-15 hashtags por post (mezclar grandes y de nicho)
- Facebook: 3-5 hashtags
- TikTok: 3-6 hashtags (priorizar trending)

---

## 7. Plantillas de Respuestas Automaticas

### 7.1 Saludo inicial (DM / Messenger)

```
Hola! Gracias por escribirnos a Espacio Nautico.

Somos especialistas en travesias en velero, escuela nautica
y compra-venta de veleros.

Contanos en que te podemos ayudar:
1. Travesias y proximas salidas
2. Escuela nautica (cursos de timonel/patron)
3. Veleros en venta
4. Servicios nauticos
5. Otro

O si preferis, escribinos directo por WhatsApp: wa.me/5491149915143
```

### 7.2 Consulta por travesias

```
Gracias por tu interes en nuestras travesias!

Nuestras proximas salidas:
[INSERTAR PROXIMAS FECHAS]

Cada travesia incluye:
- Navegacion ida y vuelta desde Puerto Norte
- Capitan y tripulacion experimentada
- Seguro nautico
- Equipamiento de seguridad completo

Para reservar o mas info, escribinos por WhatsApp: wa.me/5491149915143
O visita: espacionautico.com.ar/travesias
```

### 7.3 Consulta por escuela

```
Te interesa aprender a navegar? Genial!

Ofrecemos cursos oficiales habilitados por Prefectura Naval:
- Timonel: [DURACION/PRECIO]
- Patron de Yate: [DURACION/PRECIO]

Clases teoricas + practicas en el Rio de la Plata.
Proxima cursada: [FECHA]

Mas info: espacionautico.com.ar/escuela-nautica
WhatsApp: wa.me/5491149915143
```

### 7.4 Consulta por veleros en venta

```
Estas buscando velero? Te ayudamos!

En este momento tenemos estas unidades disponibles:
- Masti: espacionautico.com.ar/veleros-en-venta/masti
- Brama: espacionautico.com.ar/veleros-en-venta/brama
- Marejada: espacionautico.com.ar/veleros-en-venta/marejada
- Brisa: espacionautico.com.ar/veleros-en-venta/brisa

Si necesitas mas detalles o queres coordinar una visita,
escribinos por WhatsApp: wa.me/5491149915143
```

### 7.5 Respuesta a comentario positivo

```
Gracias por las buenas vibras! Nos vemos en el agua pronto.
```

Variantes:
- "Gracias! El rio siempre da momentos asi."
- "Que bueno que te guste! Te esperamos a bordo."
- "Gracias! Navegar es asi, una vez que probas no paras."

### 7.6 Respuesta a consulta de precio

```
Hola! Te mandamos todos los detalles por DM para darte la info
mas actualizada. Tambien podes escribirnos directo al WhatsApp:
wa.me/5491149915143
```

### 7.7 Escalado a humano

Cuando el bot no puede resolver, usa este mensaje:
```
Gracias por tu consulta! Tu mensaje requiere atencion personalizada.
Te vamos a responder a la brevedad, o si es urgente podes
llamarnos/escribirnos al +54 9 11 4991-5143.
```

---

## 8. Reglas de Escalado

| Tipo de mensaje | Accion |
|----------------|--------|
| Saludo simple | Bot responde automaticamente |
| Consulta estandar (travesias, escuela, veleros) | Bot responde con plantilla |
| Consulta de precio especifica | Bot deriva a humano |
| Queja o reclamo | Derivar a humano INMEDIATAMENTE |
| Propuesta comercial / sponsor | Derivar a humano |
| Consulta tecnica compleja | Derivar a humano |
| Emergencia / seguridad | Derivar a humano + alerta |

### Tiempo de respuesta objetivo
- Automatica (bot): Inmediata
- Humano (consulta simple): < 2 horas en horario laboral
- Humano (urgente): < 30 minutos
- Fuera de horario: Bot responde + "Te contestamos manana a primera hora"

---

## 9. Pautas Visuales para Redes

### Paleta de colores (segun conceptos de logo)
- **Navy/Azul oscuro** `#0A1520` / `#0D2D4A`: Color principal (fondos, textos)
- **Verde agua/Teal** `#4DB8A0` / `#3DCFB0`: Acento principal, velas, energia
- **Azul medio** `#3A9FD4` / `#1A6FA0`: Acento secundario, olas, profundidad
- **Dorado** `#D4A843`: Destacados, labels, detalles premium
- **Crema/Blanco** `#E8EDF2`: Textos sobre fondos oscuros, limpieza

### Tipografias en imagenes
- Titulos: **Teko** o **Bebas Neue** (uppercase, bold)
- Subtitulos: **Barlow Condensed** (uppercase, light)
- Cuerpo: **Outfit** (normal weight)

### Estilo fotografico
- Fotos reales (nada de stock generico)
- Luz natural, preferencia por golden hour / atardeceres
- Mostrar personas disfrutando (no solo veleros vacios)
- Angulos dinamicos (desde cubierta, desde el mastil, desde el agua)
- Buenos Aires y el rio como protagonistas

### Formatos por plataforma

| Formato | Dimensiones | Uso |
|---------|-------------|-----|
| Feed Instagram/FB | 1080x1080 px | Posts estandar |
| Story/Reel vertical | 1080x1920 px | Stories, Reels, TikTok |
| Portada FB | 820x312 px | Cover photo |
| Carrusel | 1080x1350 px | Posts tipo slider |

### Plantilla base para posts
- Logo ENBA pequeno en esquina superior (watermark sutil)
- Texto sobre foto con overlay oscuro semitransparente
- CTA en la ultima slide del carrusel
